...
def parse_company_year(pdf_file: Path) -> tuple[str, str]:
    ...
    Supports names like `2024_Apple.pdf` and `Apple_2024.pdf`.
    """
    stem = pdf_file.stem
    parts = stem.split("_")
...
def ingest_document(
    ...
    """
    Ingest a single PDF document: convert to markdown, chunk, embed,
    upload to the vector store, extract KPIs, and save them.
    """
